...
from flaskdemo.decorators import confirm_required, permission_required
from flaskdemo.emails import send_change_email_email
from flaskdemo.forms.user import EditProfileForm, UploadAvatarForm, CropAvatarForm, ChangeEmailForm, ChangePasswordForm, NotificationSettingForm, PrivacySettingForm, DeleteAccountForm
from flaskdemo.models import User, Photo, Collect
from flaskdemo.settings import Operations
from flaskdemo.utils import generate_token, validate_token, redirect_back, flash_errors
from flaskdemo.notifications import push_follow_notification
from flaskdemo.extensions import db, avatars
...
def index(username):
    ...
    if user == current_user and user.locked:
        flash("Your account is locked.", category="danger")
    if user == current_user and not user.active:
        logout_user()
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config["ALBUMY_PHOTO_PER_PAGE"]
    pagination = Photo.query.with_parent(user).order_by(Photo.timestamp.desc()).paginate(page, per_page)  # with_parent的使用
...
def upload_avatar():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Image uploaded, please crop.", category="success")
    flash_errors(form)
...
def change_email_request():
    ...
    if form.validate_on_submit():
        ...
        flash("Confirm email sent, check your inbox.", category="info")
        return redirect(url_for("user.index", username=current_user.username))
    return render_template("user/settings/change_email.html", form=form)


@user_bp.route("/change-email/<token>", methods=['GET'])
@login_required
...
def privacy_setting():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Privacy settings updated.", category="success")
        return redirect(url_for("user.index", username=current_user.username))
...
def delete_account():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Your are free, goodbye!", category="success")
